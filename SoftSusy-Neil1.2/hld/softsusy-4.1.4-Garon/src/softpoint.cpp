...
void errorCall() {
  ...
  ii << "SOFTSUSY" << PACKAGE_VERSION 
     << " called with incorrect argumentz. Need to put either:\n";
  ...
  ii << "./softpoint.x FGMCaseB1LargeNonRN (B1 in infinite beta limit+NR operator) [mGMSB params] --betaeps\n" ;
...
int main(int argc, char *argv[]) {
  ...
  if (argc == 1 || ( (strcmp(argv[1], "sugra") && 
		      ...
		      strcmp(argv[1], "FGMCaseB1LargeNonRN") &&
  ...
  printf("neil is about to call FGMBS\n");
	...
	r = &m; //telling it to flag the normal version
		    ...
		    if (!strcmp(argv[1], "FGMCaseB1LargeNonRN")) {
	cout << "# SOFTSUSY FGM CaseB1Large" << endl;
	
	flavourViolation = true;
	boundaryCondition = &FGMCaseB1LargeNonRNbcs;
	modelIdent = "FGMCaseB1LargeNonRN";
    printRuledOutSpectra = false;
	pars.setEnd(4);
  ...
  if (!strcmp(argv[1], "leshouches")) {
	...
	if (argc == 2) {
	  ...
	  while (getline(cin,line)) {
		...
		if (block == "MODSEL") {
          ...
          printf("ckecking extendedSugraBcs\n");
	...
	r->setLoops(3); k.setLoops(3); nmssm.setLoops(3);
      ...
      switch (susy_model) {
    case MSSM:
      printf("neil is in fixedPointIteration\n");
