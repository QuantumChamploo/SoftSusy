...
namespace softsusy {
  ...
  void FGMCaseB2bcs(MssmSoftsusy & m, const DoubleVector & inputParameters) {
    ...
    void FGMCaseShubcs(MssmSoftsusy & m, const DoubleVector & inputParameters) {
    double alpha  = inputParameters.display(1);
    double M0  = inputParameters.display(2);
    double nM = inputParameters.display(3);
    double nHd = inputParameters.display(4);
    double nHu = inputParameters.display(6);
    double b4 = inputParameters.display(5);
    m.FGMCaseShu(m.displayMssmSusy(), alpha, M0, nM, nHd, b4, nHu);

    return;
  }
  
      void FGMCaseShuAltbcs(MssmSoftsusy & m, const DoubleVector & inputParameters) {
    double alpha  = inputParameters.display(1);
    double M0  = inputParameters.display(2);
    double nM = inputParameters.display(3);
    double nHd = inputParameters.display(4);
    double nHu = inputParameters.display(5);
    m.FGMCaseShuAlt(m.displayMssmSusy(), alpha, M0, nM, nHd, nHu);

    return;
  }
  
    void FGMCaseShuLargebcs(MssmSoftsusy & m, const DoubleVector & inputParameters) {
    double alpha  = inputParameters.display(1);
    double M0  = inputParameters.display(2);
    double nM = inputParameters.display(3);
    m.FGMCaseShuLarge(m.displayMssmSusy(), alpha, M0, nM);

    return;
  }
    void FGMCaseShuLargeNonRNbcs(MssmSoftsusy & m, const DoubleVector & inputParameters) {
    m.FGMCaseShuLargeNonRN(m.displayMssmSusy(), inputParameters(1), inputParameters(2), inputParameters(3), inputParameters(4));
