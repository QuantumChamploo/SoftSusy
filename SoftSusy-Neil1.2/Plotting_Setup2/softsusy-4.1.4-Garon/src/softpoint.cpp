...
void errorCall() {
  ...
  ii << "./softpoint.x FGMCaseShu [mGMSB params] [FGM params]\n" ;
  ii << "./softpoint.x FGMCaseBNonRN1 [mGMSB params] [FGM params] --betaeps\n" ;
  ii << "./softpoint.x FGMCaseBNonRN2 [mGMSB params] --beta2 --betaeps --thetavev\n" ;
  ii << "./softpoint.x FGMCaseBNonRN3 [mGMSB params] [FGM params] --betaeps --thetavev\n" ;
  ii << "./softpoint.x FGMCaseShuLarge (B1 in infinite beta limit) [mGMSB params]\n" ;
  ii << "./softpoint.x FGMCaseShuLargeNonRN (B1 in infinite beta limit+NR operator) [mGMSB params] --betaeps\n" ;
  ii << "./softpoint.x FGMCaseShuAlt (Root 2 eigenvector in 2nd Column) [mGMSB params] [FGM params]\n" ;
...
int main(int argc, char *argv[]) {
  ...
  double m0 = 0., m12 = 0., a0 = 0., m32 = 0., mMess = 0., n5 = 0.,
    LAMBDA = 0., cgrav = 1., beta2 = 0., epsd = 0., epsu = 0., epse = 0., beta3 = 0., beta4 = 0., betaeps = 0., thetavev=0.;
  ...
  if (argc == 1 || ( (strcmp(argv[1], "sugra") && 
		      ...
		      strcmp(argv[1], "FGMCaseShu") &&
		      strcmp(argv[1], "FGMCaseShuLarge") &&
		      strcmp(argv[1], "FGMCaseShuLargeNonRN") &&
		      strcmp(argv[1], "FGMCaseShuAlt") &&
  ...
  if (strcmp(argv[1], "leshouches")) {
    ...
    else if (starts_with(argv[i], "--beta4=")){
    beta4 = get_value(argv[i], "--beta4=");}
    else if (starts_with(argv[i], "--epsu=")){
    epsu = get_value(argv[i], "--epsu=");}
    else if (starts_with(argv[i], "--epsd=")){
    epsd = get_value(argv[i], "--epsd=");}
    else if (starts_with(argv[i], "--epse=")){
    epse = get_value(argv[i], "--epse=");}
	...
	r = &m; //telling it to flag the normal version 
	}
	    if (!strcmp(argv[1], "FGMCaseShu")) {
	cout << "# SOFTSUSY FGM CaseB1" << endl;
	
	flavourViolation = true;
	boundaryCondition = &FGMCaseShubcs;
	modelIdent = "FGMCaseShu";
    printRuledOutSpectra = false;
	pars.setEnd(6);
	pars(1) = LAMBDA; pars(2) = mMess; pars(3) = epsu; pars(4) = epsd; pars(5) = epse; pars(6) = cgrav;
	if (mMess < 1000.) {
	  ostringstream ii; 
	  ii << " mMess=" << mMess
	     << " in FGM input (too low). The point will not yield a sensible answer\n";
	...
	r = &m; //telling it to flag the normal version
	    ...
	    if (!strcmp(argv[1], "FGMCaseShuLarge")) {
	cout << "# SOFTSUSY FGM CaseB1Large" << endl;
	
	flavourViolation = true;
	boundaryCondition = &FGMCaseShuLargebcs;
	modelIdent = "FGMCaseShuLarge";
    printRuledOutSpectra = false;
	pars.setEnd(3);
	pars(1) = LAMBDA; pars(2) = mMess; pars(3) = cgrav;
	if (mMess < 1000.) {
	...
	r = &m; //telling it to flag the normal version
		    ...
		    if (!strcmp(argv[1], "FGMCaseShuLargeNonRN")) {
	cout << "# SOFTSUSY FGM CaseB1Large" << endl;
	
	flavourViolation = true;
	boundaryCondition = &FGMCaseShuLargeNonRNbcs;
	modelIdent = "FGMCaseShuLargeNonRN";
    printRuledOutSpectra = false;
	pars.setEnd(4);
  ...
  r = &m; //telling it to flag the normal version
		    ...
		    if (!strcmp(argv[1], "FGMCaseShuAlt")) {
	cout << "# SOFTSUSY FGM CaseB1" << endl;
	
	flavourViolation = true;
	boundaryCondition = &FGMCaseShuAltbcs;
	modelIdent = "FGMCaseShuAlt";
    printRuledOutSpectra = false;
	pars.setEnd(5);
